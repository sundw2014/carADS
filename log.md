...
1.多源问题，即可能会发生的同时赋值问题，比如在两个always中同时写了给某个寄存器赋值的语句，而且这两个语句可能同时发生。  
比较诡异的是编译器的行为，可不可综合有时会和自己的推理不同。有时候编译器会很机智地优化代码语义（见0x02中的那个问题）使其综合成功，有时候就不很科学。
2.always里面的代码if条件与always敏感条件的关系
### 下午
给蓝牙加上了切换控制模式的功能，但是因为一个3'd8耽误了很长时间
### 晚上
1.bdf里面的连线默认是input或是output，inout类型需要自己注意才行
2.字面值在书写的时候注意size要写对,比如3'd8就很愚蠢
3.给小车加上了避障功能，遇到近距离障碍物就右转，近距离无障碍物就直行
4.超声波模块有问题，太远的echo太长，就出错了
## 0x08 周四
### 上午
验收完成
bug list:
1.app点击“断开”会崩掉，不管了，反正app不是重点。
2.PCB的电源芯片选用了LM084,这是一个LDO，效率随着压差升高而降低，所以发热严重，在我们这个应用场景里，文波没有要求，所以该用开关电源芯片。
