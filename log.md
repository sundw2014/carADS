...
小车属于 ，依靠T形板提供后悬挂回复力，但是小车的T形板竟然与车体只有一个螺丝。。。于是打了两个孔，放了两个机米螺丝限位。  
...
## 0x05 周一
### 上午
焊板子，调试板子，板子没有问题，只是LDO发热严重。  
把板子和FPGA连在一起调试，用蓝牙控制，感觉很好，很流畅。
### 下午
去采购了新的IMU模块，浪费了一下午。

## 0x06 周二
### 上午
给小车做了一个支架，把FPGA放上去固定住，然后换了一个电池，小车动力更强了。  
这样小车可以用蓝牙控制着在地上跑来跑去了，而且延迟比预想的小。
### 下午
问题： 如何在verilog里实现一个数据选择器（组合电路）
答： always之外不能出现if语句，但是条件运算是允许的。hack如``` assign dout=se1?se2?d4:d3:se2?d2:d1; ```

## 0x07 周三
开始两部分代码联调
### 上午
一上午都在编译代码，各种编译错误。常见的，值得思考的几个错误如下  
+ 多源问题，即可能会发生的同时赋值问题，比如在两个always中同时写了给某个寄存器赋值的语句，而且这两个语句可能同时发生。  
比较诡异的是编译器的行为，可不可综合有时会和自己的推理不同。有时候编译器会很机智地优化代码语义（见0x02中的那个问题）使其综合成功，有时候就不很科学。
+ always里面的代码if条件与always敏感条件的关系
### 下午
给蓝牙加上了切换控制模式的功能，但是因为一个3'd8耽误了很长时间
### 晚上
+ bdf里面的连线默认是input或是output，inout类型需要自己注意才行
+ 字面值在书写的时候注意size要写对,比如__3'd8__就很愚蠢
+ 给小车加上了避障功能，遇到近距离障碍物就右转，近距离无障碍物就直行
+ 加入了上电复位，本来是想用电容来实现的，可是时间不够了，就用代码实现了,如下  
...
+ 超声波模块有问题，太远的echo太长，就出错了

## 0x08 周四
### 上午
验收完成
bug list:
+ app点击“断开”会崩掉，不管了，反正app不是重点。
+ PCB的电源芯片选用了LM084,这是一个LDO，效率随着压差升高而降低，所以发热严重，在我们这个应用场景里，文波没有要求，所以该用开关电源芯片。
